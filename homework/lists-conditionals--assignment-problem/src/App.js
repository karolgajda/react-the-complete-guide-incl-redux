...
class App extends Component {
    ...
    removeCharHandler = (index) => {
        const test = [...this.state.inputText]
        test.splice(index, 1)
        this.setState({inputText: test.join('')});
    }

    render() {

        const chars = this.state.inputText.split('').map(
        ...
        return (
            <div className="App">
                ...
                <input onChange={(event) => this.changeTextHandler(event)} value={this.state.inputText}/>
                <p>Your text:</p>
